...
;layui.define(["layer", "laytpl", "upload"], function (i) {
    ...
    var u = function (i) {
            ...
            return i = i || {},
                ...
                ["{{# var length = 0; layui.each(" + i.item + ", function(i, data){ length++; }}",
                    ...
                    '<span class="layui-icon layim-status-hide"  lay-type="show">&#xe60f;</span>',
                    "{{# } }}",
                    '{{ data.username||data.groupname||data.name||"佚名" }}</span><p>{{ data.remark||data.sign||"" }}</p><span class="layim-msg-status">new</span></li>',
                    "{{# }); if(length === 0){ }}",
                    '<li class="layim-null">' + (a[i.type] || "暂无数据") + "</li>", "{{# } }}"].join("")
        },
        y = ['<div class="layui-layim-main">',
            ...
            '<li {{d.mine.status === "hide" ? "class=layim-this" : ""}} layim-event="status" lay-type="hide"><i class="layui-icon">&#xe605;</i><cite class="layui-icon layim-status-hide">&#xe60f;</cite>隐身</li>', "</ul>",
            "</div>", '<input class="layui-layim-remark" placeholder="编辑签名" lay-event="sign" value="{{ d.mine.remark||d.mine.sign||"" }}">',
            "</div>", '<ul class="layui-unselect layui-layim-tab{{# if(!d.base.isfriend || !d.base.isgroup){ }}',
            " layim-tab-two", '{{# } }}">',
            '<li class="layui-icon',
            "{{# if(!d.base.isfriend){ }}",
            " layim-hide", "{{# } else { }}",
            ...
            "</li>", "</ul>",
            '<ul class="layui-unselect layim-tab-content  {{# if(!d.base.isfriend && !d.base.isgroup){ }}layui-show{{# } }}">',
            "<li>", '<ul class="layui-layim-list layui-show layim-list-history">', u({type: "history"}),
            "</ul>", "</li>",
            "</ul>", '<ul class="layui-unselect layim-tab-content">',
            "<li>", '<ul class="layui-layim-list layui-show" id="layui-layim-search"></ul>', "</li>", "</ul>",
            ...
            "{{# if(!d.base.copyright){ }}", '<li class="layui-icon layim-tool-about" layim-event="about" title="关于">&#xe60b;</li>',
        ...
        f = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">',
            ...
            '<div class="layim-chat-main">', "<ul></ul>",
            "</div>", '<div class="layim-chat-footer">', '<div class="layui-unselect layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">',
        ...
        p = ['<div class="layim-add-box">',
            ...
            '<div class="layim-add-remark">', '{{# if(d.data.type === "friend" && d.type === "setGroup"){ }}',
            "<p>选择分组</p>",
            '{{# } if(d.data.type === "friend"){ }}', '<select class="layui-select" id="LAY_layimGroup">',
            "{{# layui.each(d.data.group, function(index, item){ }}", '<option value="{{ item.id }}">{{ item.groupname }}</option>',
            "{{# }); }}", "</select>", "{{# } }}", '{{# if(d.data.type === "group"){ }}', "<p>请输入验证信息</p>",
            '{{# } if(d.type !== "setGroup"){ }}', '<textarea id="LAY_layimRemark" placeholder="验证信息" class="layui-textarea"></textarea>',
            "{{# } }}",
            "</div>", "</div>"].join(""),
        h = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>',
            '<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>',
            "{{# if(d.mine){ }}", '<i>{{ layui.data.date(d.timestamp) }}</i>{{ d.username||"佚名" }}',
            "{{# } else { }}", '{{ d.username||"佚名" }}<i>{{ layui.data.date(d.timestamp) }}</i>',
            "{{# } }}",
            "</cite></div>", '<div class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp") }}</div>',
            "</li>"].join(""),
    ...
    layui.data.date = function (i) {
        ...
        layui.data.content = function (i) {
            ...
            return i = (i || "").replace(/&(?!#?[a-zA-Z0-9]+;)/g, "&amp;")
    ...
    var x, b, w, k, C,
        ...
        j = {message: {}, chat: []}, H = function (i) {
            ...
            return mine = a.mine || {}, local = layui.data("layim")[mine.id] || {},
                ...
                var t = a.mine || {}, l = layui.data("layim")[t.id] || {}, s = {
                    ...
                    local: l, //本地数据
                    mine: t,//我的用户信息
                    friend: a.friend || [], //联系人信息
                    group: a.group || [],//群组信息
                    history: l.history || {}//历史会话信息
                };
                j = e.extend(j, s),
                    L(n(y).render(s)), (l.close || i.min) && T(),
        ...
        //主面板最小化状态
        T = function (i) {
            return b && t.close(b.attr("times")), x && x.hide(),
                j.mine = j.mine || {},
                t.open({
                    type: 1,
                    title: !1,
        ...
        z = function (i) {
            ...
            if (a[0]) {
                ...
                1 !== s.find("li").length || o[0] || (d || w.css("width", 800),
                    ...
                    //打开的是非当前聊天面板，则新增面板
                o[0] || (s.append(n(v).render(l)), c.append(n(f).render(l)),
                    I(i), E()), O(s.find(".layim-chatlist-" + i.type + i.id)),
                o[0] || U(), $(i), Q(),
                    C
            }
            l.first = !0;
            var u = C = t.open({
                ...
                success: function (a) {
                    w = a, a.css({
                        "min-width": "500px",
                        "min-height": "420px"
                    }), I(i), "function" == typeof i.success && i.success(a), Q(), R(a), $(i), U(), q(),
                        //聊天窗口的切换监听
        ...
        O = function (i, a) {
            ...
            if (a) {
                ...
                //只剩下1个列表，隐藏左侧区块 聊天列表不存在，则关闭聊天界面
                ...
                layui.each(t.list, function (t, n) {
                    if (i.id == n.id) {
                        ...
                        return l.find(".layui-layim-list>li").eq(t).remove(),
                            j.friend[e].list.splice(t, 1), //从cache的friend里面也删除掉好友
                            l.find(".layim-count").html(j.friend[e].list.length),//刷新好友数量
                            //如果一个好友都没了
                        0 === j.friend[e].list.length && l.find(".layui-layim-list").html('<li class="layim-null">该分组下已无好友了</li>'), !0
                    }
                })
            }) : "group" === i.type && layui.each(j.group, function (e, t) {
                if (i.id == t.id)
                    return a.find(">li").eq(e).remove(),
                        //从cache的group里面也删除掉数据
                        j.group.splice(e, 1),
                        //如果一个群组都没了
                    0 === j.group.length && a.html('<li class="layim-null">暂无群组</li>'), !0
            }))
        },
        //查看更多记录
        Z = function () {
            var i = _(), a = i.elem.find(".layim-chat-main"),
                e = a.find("ul"), t = e.find("li").length;
            if (t >= d) {
